...
export default function Electronics(props) {
  ...
  const user=AuthService.getCurrentUser()
    ...
    useEffect(()=>{
      axios.get("http://localhost:8080/api/vlx/getBikes").then(res=>SetProducts(res.data.filter(pro=>pro.category==="Home Electronics"))).catch(err=>alert(err))
    },[])

    function handleFilter(city) {

      // SetProducts(product.filter((pro)=>pro.place===city))
      axios.get("http://localhost:8080/api/vlx/getBikes").then(res=>SetProducts(res.data.filter((pro)=>pro.place===city && pro.category==="Home Electronics" ))).catch(err=>alert(err));
    }
    ...
    function handleCostFilter(str) {
      if(str=="below5L")
      {
        axios.get("http://localhost:8080/api/vlx/getBikes").then(res=>SetProducts(res.data.filter((pro)=>pro.price < 10000 && pro.category==="Home Electronics" ))).catch(err=>alert(err));
...
<div  style={{display:"flex",flexDirection:"column"}}>
   <div className="Car__filters__location">
     ...
     >
       ...
       <Tab label="Below 10k" value="below5L" onClick={(e)=>{
         handleCostFilter("below5L")
      
        
       }} />
       <Tab label="above 10k" onClick={(e)=>{
        e.preventDefault();
