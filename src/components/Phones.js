...
export default function Phones(props) {
  ...
  const user=AuthService.getCurrentUser()
    ...
    useEffect(()=>{
      axios.get("http://localhost:8080/api/vlx/getBikes").then(res=>SetProducts(res.data.filter(pro=>pro.category==="Mobile phones"))).catch(err=>alert(err))
    },[])

    function handleFilter(city) {

      // SetProducts(product.filter((pro)=>pro.place===city))
      axios.get("http://localhost:8080/api/vlx/getBikes").then(res=>SetProducts(res.data.filter((pro)=>pro.place===city && pro.category==="Mobile phones" ))).catch(err=>alert(err));
    }
    ...
    function handleCostFilter(str) {
      if(str=="below5L")
      {
        axios.get("http://localhost:8080/api/vlx/getBikes").then(res=>SetProducts(res.data.filter((pro)=>pro.price < 10000 && pro.category==="Mobile phones" ))).catch(err=>alert(err));
